...
class DigitConv(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return F.softmax(x, dim=1)[:, :4].contiguous()
...
def main():
    ...
    save = 'sudoku4{}{}.boardSz{}{}{}-{}-aux{}-m{}-lr{}-bsz{}'.format(
        ...
        args.boardSz, '-sparse' if args.sparse else '', args.threshold if args.sparse else '', args.model, args.aux, args.m, args.lr, args.batchSz)
    ...
    # with open(os.path.join('data/sudoku4data', 'features_img.pt'), 'rb') as f:
    Ximg_in = torch.load(os.path.join('data/sudoku4data', 'features_img.pt'))
    # print(X_in.shape, Y_in.shape) # N, 4, 4, 4
    # print(X_in[0], Y_in[0])
    # input()
    ...
    # print(Y_in.shape) # 85632, 4, 4, 4
    # print(Y_in[0])
    # input()
    # print(Ximg_in.shape)
    # input()
    X, Ximg, Y, is_input = process_inputs(X_in, Ximg_in, Y_in, 2)
    ...
    global_mask = torch.ones((65+args.aux, 65+args.aux), dtype=torch.int, device=X.device)
    global_mask.fill_diagonal_(0)
    for epoch in range(1, args.nEpoch + 1):
        train(args.boardSz, epoch, model, optimizer, train_logger, train_set, args.batchSz, unperm, log_file)
        if args.sparse:
            assert args.model == 'mixnet'
            # if args.mnist and torch.norm(model.sudoku_solver.C.data - old_C) < 0.01 and torch.count_nonzero(model.sudoku_solver.C.data) > model.sudoku_solver.C.data.numel() / 2:
            ...
            #     model.sudoku_solver.C.data = model.sudoku_solver.C.data * mask
            # elif torch.norm(model.C.data - old_C) < 0.01 and torch.count_nonzero(model.C.data) > model.C.data.numel() / 2:
            #     nonzero_idx = torch.nonzero(model.C.data, as_tuple=True)
            #     min_v = torch.min(torch.abs(model.C.data[nonzero_idx]))
            #     mask = (torch.abs(model.C.data) != min_v)
            #     model.C.data = model.C.data * mask
            if args.mnist:
                old_C = model.sudoku_solver.C.data.clone() * global_mask
                threshold = args.threshold * old_C.abs().sum() / torch.count_nonzero(global_mask)
                mask = (torch.abs(old_C) >= threshold).int()
                global_mask *= mask
                model.sudoku_solver.C.data = old_C * global_mask
                assert torch.count_nonzero(global_mask) == torch.count_nonzero(model.C.data)
                print(f'non-zero elements: {torch.count_nonzero(model.sudoku_solver.C.data).item()}')
            else:
                old_C = model.C.data.clone() * global_mask
                threshold = args.threshold * old_C.abs().sum() / torch.count_nonzero(global_mask)
                mask = torch.abs(old_C) >= threshold
                global_mask *= mask
                model.C.data = old_C * global_mask
                assert torch.count_nonzero(global_mask) == torch.count_nonzero(model.C.data)
                print(f'non-zero elements: {torch.count_nonzero(model.C.data).item()}')
            # with open('debug.txt', 'a') as ff:
            #     print(torch.count_nonzero(old_C), file=ff)
            with open(log_file, 'a') as f:
                print(f'non-zero elements: {torch.count_nonzero(model.C.data).item()}', file=f)
